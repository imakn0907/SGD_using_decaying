...
class WideResNet_for_CIFAR10(nn.Module):
    def __init__(self, depth, widen_factor, dropRate=0.0, num_classes=10):
        super(WideResNet_for_CIFAR10, self).__init__()
...
class WideResNet_for_CIFAR100(nn.Module):
    def __init__(self, depth, widen_factor, dropRate=0.0, num_classes=100):
        super(WideResNet_for_CIFAR100, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        assert((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = BasicBlock
